 ...
 * </p>
 */
public class DemoExternalRealtimeTradingProvider_2 extends DemoExternalRealtimeProvider {
...
//		Log.info("***simpleParameters = " + simpleParameters.toString());
//		Log.info("***simpleParameters.stopPrice = " + simpleParameters.stopPrice);
//		Log.info("***simpleParameters.limitPrice = " + simpleParameters.limitPrice);
...
//		Log.info("***OrderInfoBuilder builder CREATED");
		...
		final OrderInfoBuilder builder = new OrderInfoBuilder(simpleParameters.alias,
				...
				"o" + orderIdGenerator.incrementAndGet(), simpleParameters.isBuy, orderType, simpleParameters.clientId,
...
//			Log.info("***Order gets REJECTED");
			...
			connr.processNewOrder(simpleParameters);
...
//				}
//			}
//			String symb = sb.toString();
//			String sid = simpleParameters.isBuy ? "Buy" : "Sell";
//			double price = simpleParameters.limitPrice;
//			double orderQty = simpleParameters.size;
//			
//			JsonObject json = new JsonObject();
//			json.addProperty("symbol", symb);
//			json.addProperty("Side", sid);
//			json.addProperty("price", price);
//			json.addProperty("orderQty", orderQty);
//			json.addProperty("ordType", "Limit");
//			String data = json.toString();
//			
////			String data = "?symbol=" + symb + "&side=" + sid + "&price=" + price + "&orderQty=" + orderQty
////					+ "&ordType=Limit";
//			long moment = System.currentTimeMillis() + 10000000;
//			String reguOrderKey = "PLc0jF_9Jh2-gYU6ye-6BS4q";
//			String secretOrderKey = "xyMWpfSlONCWCwrntm0GotQN42ia291Vv2aWANlp-f0Kb5-I";
////			String messageBody = connr.createMessageBody("POST", path + data, data, moment);
//			String messageBody = connr.createMessageBody("POST", path, data, moment);
//			String signature;
//			try {
//				signature = connr.generateSignature(secretOrderKey, messageBody);
//				String answer = connr.post(connr.restApi + path, reguOrderKey, signature, moment, data);
////				String answer = connr.post(connr.restApi + path + data, reguOrderKey, signature, moment, data);
//				Log.info(answer);
...
//		Log.info("*******simulateOrders*******");
		...
		synchronized (workingOrders) {
			synchronized (instruments) {
				...
				// ************************
				// workingOrders.values().removeIf(o -> o.getStatus() !=
				// OrderStatus.WORKING);
				// List<OrderInfoBuilder> oibList = new ArrayList<>();
				// for (OrderInfoBuilder oib : workingOrders.values()) {
				// if (oib.getStatus() != OrderStatus.WORKING) {
				// oibList.add(oib);
				// }
				// }
				// for (OrderInfoBuilder oib : oibList) {
				// workingOrders.values().remove(oib);
				// }
				// ************************
...
//					Log.info("***instrument = " + instrument.toString());
					...
					if (instrument != null) {
						...
						double bestPrice = order.isBuy ? instrument.getBestAsk() * instrument.pips
								: instrument.getBestBid() * instrument.pips;
						
//						Log.info("***bestPrice = " + bestPrice);

						boolean shouldBeExecuted = order.isBuy ? bestPrice <= order.getLimitPrice()
								: bestPrice >= order.getLimitPrice();

//						Log.info("***shouldBeExecuted = " + shouldBeExecuted);
